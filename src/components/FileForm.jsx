import { useState, useRef } from "react";

const FileForm = ({ order, uploadFile, loading, uploadError, setUploadError }) => {
    const [pdf, setPdf] = useState(null);

    const fileInput = useRef();

    const handleChange = (e) => {
        setPdf(e.target.files[0]);
        setUploadError('')
    };

    const handleSubmit = (e) => {
        e.preventDefault();
        uploadFile(pdf, order.id);
        fileInput.current.value = "";
    };
    return (
        <>
            <div className="flex items-center justify-center">
                <div className="mt-10 mb-10 border-2 border-dashed text-center w-4/5">
                    <div className="mt-10">
                        <form onSubmit={handleSubmit}>
                            <div className="mt-4">
                                <label>Add a file to the merge</label>
                            </div>
                            {loading ? (
                                <div className="flex place-content-center mt-2 mb-2">
                                    <span>Uploading </span>
                                    <span className="loading loading-dots loading-xs text-primary m-2"></span>
                                </div>
                            ) : (
                                <>
                                    <div className="mt-4">
                                        <input type="hidden" name="order" />
                                        <input
                                            type="file"
                                            className="file-input file-input-bordered file-input-outline w-full max-w-xs"
                                            placeholder="add a file to merger"
                                            ref={fileInput}
                                            onChange={(e) => handleChange(e)}
                                            name="filename"
                                        />
                                    </div>
                                    <div className="mt-4">
                                        <button
                                            type="submit"
                                            className="btn btn-neutral btn-outline mb-5"
                                        >
                                            Upload
                                        </button>
                                    </div>
                                </>
                            )}
                            {uploadError &&
                            <div className="text-error mt-2 mb-2">
                                {/* <svg xmlns="http://www.w3.org/2000/svg" className="stroke-current shrink-0 h-6 w-6" fill="none" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg> */}
                                <span>{uploadError}</span>
                            </div>
                        }
                        </form>
                    </div>
                </div>
            </div>
        </>
    );
};
export default FileForm;
